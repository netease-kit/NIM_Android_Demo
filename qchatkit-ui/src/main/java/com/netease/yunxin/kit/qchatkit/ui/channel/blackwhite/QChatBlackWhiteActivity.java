...
/**
 * black and white name list page
 * show the member list in channel,when the channel type is public it is black list
 * if the channel type is private,it is white list
 */
public class QChatBlackWhiteActivity extends CommonListActivity {
    ...
    public void initData() {
        ...
        viewModel.getResultLiveData().observe(this, result -> {
            if (result.getLoadStatus() == LoadStatus.Success) {
                ...
                Toast.makeText(this, getResources().getString(result.errorMsg().getRes()), Toast.LENGTH_SHORT).show();
                ALog.d(TAG, "ResultLiveData", "Error");
            }
        });
        //add member live data
        viewModel.getAddLiveData().observe(this, result -> {
            if (result.getLoadStatus() == LoadStatus.Success) {
                loadData();
    ...
    public String getTitleText() {
        return channelType == QChatChannelModeEnum.Public ? getResources().getString(R.string.qchat_channel_black_name_list) : getResources().getString(R.string.qchat_channel_white_name_list);
    }
    ...
    public CommonViewHolder<QChatBaseBean> onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == QChatViewType.SERVER_MEMBER_VIEW_TYPE) {
            QChatChannelNameListMemberViewHolderBinding viewBinding = QChatChannelNameListMemberViewHolderBinding.inflate(getLayoutInflater(), parent, false);
            MemberViewHolder memberViewHolder = new MemberViewHolder(viewBinding);
            memberViewHolder.setOnClickListener((data, position) -> {
                if (data instanceof QChatChannelMemberBean && !editStatus) {
                    ...
                    bundle.putSerializable(QChatConstant.CHANNEL_MEMBER, ((QChatChannelMemberBean) data).channelMember);
                    intent.putExtras(bundle);
                    startActivity(intent);
                }
            });
            memberViewHolder.setDeleteClickListener((data, position) -> {
            ...
            QChatNameListAddMemberViewHolderBinding viewBinding = QChatNameListAddMemberViewHolderBinding.inflate(getLayoutInflater(), parent, false);
            AddMemberViewHolder addMemberViewHolder = new AddMemberViewHolder(viewBinding);
            addMemberViewHolder.setItemOnClickListener((data, position) -> {
                Intent intent = new Intent(QChatBlackWhiteActivity.this, QChatMemberSelectorActivity.class);
    ...
    private void registerResult() {
        selectorListLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() == RESULT_OK && result.getData() != null) {
                ArrayList<QChatServerRoleMemberInfo> members = result.getData().getParcelableArrayListExtra(REQUEST_MEMBER_SELECTOR_KEY);
    ...
    private void showDeleteDialog(QChatServerMemberInfo data, int position) {
        ...
        String content = String.format(getResources().getString(R.string.qchat_channel_name_list_delete_content), data.getNick());
        ...
        commonConfirmDialog
                ...
                .setConfirmListener(new ChoiceListener() {
                    ...
                    public void onPositive() {
                        ...
                        viewModel.deleteMember(serverId, channelId, channelType.ordinal(), data.getAccId(), position);
                    }
                })
