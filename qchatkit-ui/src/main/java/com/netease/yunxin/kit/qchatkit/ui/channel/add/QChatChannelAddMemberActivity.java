...
/**
 * add member to channel permission page
 * show the server member list
 */
public class QChatChannelAddMemberActivity extends CommonListActivity {
    ...
    public CommonViewHolder<QChatBaseBean> onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == QChatViewType.SERVER_MEMBER_VIEW_TYPE) {
            QChatChannelMemberViewHolderBinding viewBinding = QChatChannelMemberViewHolderBinding.inflate(getLayoutInflater(), parent, false);
            ServerMemberViewHolder addRoleViewHolder = new ServerMemberViewHolder(viewBinding);
            addRoleViewHolder.setItemOnClickListener((data, position) -> {
                if (data instanceof QChatServerMemberBean) {
                    viewModel.addMemberToChannel(serverId, channelId, ((QChatServerMemberBean) data).serverMember.getAccId());
                }
            });
