 ...
 * modify member's channel permission
 */
public class QChatMemberPermissionActivity extends BaseActivity {
    ...
    private void initView() {
        viewBiding.qChatMemberPermissionTitleBar.setTitle(R.string.qchat_channel_member_permission_title);
        viewBiding.qChatMemberPermissionTitleBar.setOnBackIconClickListener(view -> {
            finish();
        });
        if (channelMember != null) {
            viewBiding.qChatMemberPermissionAvatarIv.setData(channelMember.getAvatarUrl(), channelMember.getNickName(), ColorUtils.avatarColor(channelMember.getAccId()));
    ...
    private void loadPermissionData(){
        ...
        if (auth.size() > 0) {
            ...
            if (optionManagerRole != null) {
                ...
                viewBiding.qChatMemberPermissionAttrSwitch.setOnClickListener(view -> switchChange(QChatRoleResourceEnum.MANAGE_CHANNEL, viewBiding.qChatMemberPermissionAttrSwitch.getCurrentStatus()));
            }
            ...
            if (optionManagerChannel != null) {
                ...
                viewBiding.qChatMemberPermissionChannelSwitch.setOnClickListener(view -> switchChange(QChatRoleResourceEnum.MANAGE_ROLE, viewBiding.qChatMemberPermissionChannelSwitch.getCurrentStatus()));
            }
            ...
            if (optionSendMsg != null) {
                ...
                viewBiding.qChatMemberPermissionSendMessageSwitch.setOnClickListener(view -> switchChange(QChatRoleResourceEnum.SEND_MSG, viewBiding.qChatMemberPermissionSendMessageSwitch.getCurrentStatus()));
            }

            QChatRoleOptionEnum managerMemberOption = auth.get(QChatRoleResourceEnum.MANAGE_BLACK_WHITE_LIST);
            if (managerMemberOption != null) {
                viewBiding.qChatMemberPermissionManagerMemberSwitch.setStatus(managerMemberOption.ordinal());
                viewBiding.qChatMemberPermissionManagerMemberSwitch.setOnClickListener(view -> switchChange(QChatRoleResourceEnum.MANAGE_BLACK_WHITE_LIST, viewBiding.qChatMemberPermissionManagerMemberSwitch.getCurrentStatus()));
            }
        }
    }
    ...
    private void switchChange(QChatRoleResourceEnum type, int value) {
        ...
        viewModel.updateMemberRole(channelMember.getServerId(), channelMember.getChannelId(), channelMember.getAccId(), valueMap);
    }
