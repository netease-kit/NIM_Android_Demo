...
/**
 * Adapter for recyclerView in {@link com.netease.yunxin.kit.qchatkit.ui.server.QChatServerFragment}.
 */
public class QChatFragmentServerAdapter extends QChatCommonAdapter<QChatFragmentServerInfo, QChatFragmentServerListItemBinding> {
    ...
    private OnClickListener<QChatFragmentServerInfo, QChatFragmentServerListItemBinding> clickListener;
    ...
    public void setItemClickListener(OnClickListener<QChatFragmentServerInfo, QChatFragmentServerListItemBinding> listener) {
    ...
    public void updateData(QChatServerInfo info) {
        ...
        int index = dataSource.indexOf(QChatFragmentServerInfo.generateWithServerId(info.getServerId()));
        if (index >= 0) {
    ...
    @Override
    public void onBindViewHolder(@NonNull ItemViewHolder<QChatFragmentServerListItemBinding> holder, int position, QChatFragmentServerInfo data) {
        super.onBindViewHolder(holder, position, data);

        QChatFragmentServerListItemBinding binding = holder.binding;
        binding.cavIcon.setData(data.serverInfo.getIconUrl(), data.serverInfo.getName(), ColorUtils.avatarColor(data.serverInfo.getServerId()));
        binding.ivMsgTip.setVisibility(ObserverUnreadInfoResultHelper.hasUnreadMsg(data.serverInfo.getServerId())? View.VISIBLE : View.GONE);
        ...
        if (data.unreadInfoItemMap == null) {
            QChatUnreadInfoSubscriberHelper.fetchServerUnreadInfoCount(data.serverInfo, result -> {
                data.unreadInfoItemMap = ObserverUnreadInfoResultHelper.getMapFromResult(data.serverInfo.getServerId(), result);
    ...
    @Override
    public void onBindViewHolder(@NonNull ItemViewHolder<QChatFragmentServerListItemBinding> holder, int position, @NonNull List<Object> payloads) {
    ...
    private void updateFocus(ItemViewHolder<QChatFragmentServerListItemBinding> holder, int position, QChatFragmentServerInfo data) {
        QChatFragmentServerListItemBinding binding = holder.binding;
        if (lastFocusPosition == position) {
