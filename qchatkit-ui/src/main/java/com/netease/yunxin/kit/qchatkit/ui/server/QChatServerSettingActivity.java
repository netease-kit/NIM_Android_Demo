...
public class QChatServerSettingActivity extends CommonActivity {
    ...
    public void initView() {
        ...
        binding.title.setTitle(R.string.qchat_server_setting)
                ...
                .setActionListener(v -> {
                    ...
                    QChatServerRepo.updateServer(serverInfo.getServerId(), name, avatarUrl, jsonObject.toString(), new QChatCallback<Void>(getApplicationContext()));
                    finish();
                });

        binding.rlyMember.setOnClickListener(v -> QChatServerMemberListActivity.launch(this, serverInfo.getServerId()));

        binding.rlyTeam.setOnClickListener(v -> {
    ...
    private void gotoPicture(View v) {
        ...
        new PhotoChoiceDialog(QChatServerSettingActivity.this).show(new FetchCallback<String>() {
            @Override
            public void onSuccess(@Nullable String param) {
                avatarUrl = param;
                binding.avatar.setData(param, serverInfo.getName(), ColorUtils.avatarColor(serverInfo.getServerId()));
            }
    ...
    public void initData() {
        ...
        boolean isOwner = TextUtils.equals(serverInfo.getOwner(), LoginService.INSTANCE.imAccId());
        if (isOwner) {
        ...
        binding.avatar.setData(serverInfo.getIconUrl(), serverInfo.getName(), ColorUtils.avatarColor(serverInfo.getServerId()));
        binding.tvServerName.setText(serverInfo.getName());
        binding.tvId.setText(String.format(getString(R.string.qchat_server_id), serverInfo.getServerId()));
    ...
    private void showConfirmDialog(boolean isOwner) {
        ...
        commonConfirmDialog
                .setTitleStr(isOwner ? getString(R.string.qchat_delete_server) : getString(R.string.qchat_leave_server))
                ...
                .setConfirmListener(new ChoiceListener() {
                    ...
                    public void onPositive() {
                        QChatCallback<Void> callback = new QChatCallback<Void>(getApplicationContext()) {
                            @Override
