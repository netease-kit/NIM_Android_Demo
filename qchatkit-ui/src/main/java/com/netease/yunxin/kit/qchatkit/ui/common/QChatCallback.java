...
public class QChatCallback<T> implements FetchCallback<T> {
    ...
    public static void showToast(int code, Context context) {
        if (code == NO_PERMISSION) {
            Toast.makeText(context, context.getString(R.string.qchat_no_permission), Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(context, context.getString(R.string.qchat_server_request_fail) + code, Toast.LENGTH_SHORT).show();
        }
    }

    @Override
    public void onException(@Nullable Throwable exception) {
        Toast.makeText(context, context.getString(R.string.qchat_server_request_fail) + exception, Toast.LENGTH_SHORT).show();
        if (process != null) {
            process.setVisibility(View.GONE);
        }
    }
