...
public abstract class CommonListActivity extends BaseActivity implements CommonViewHolderFactory<QChatBaseBean>{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        viewBinding.commonActRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                if(newState == RecyclerView.SCROLL_STATE_IDLE){
                    ...
                    if(isLoadMore() && recyclerViewAdapter.getItemCount() < position + LOAD_MORE_DIFF){
                        QChatBaseBean last = recyclerViewAdapter.getData(recyclerViewAdapter.getItemCount() - 1);
                        loadMore(last);
                    }
                }
            }
