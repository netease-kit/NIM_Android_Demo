...
public class QChatRoleSortActivity extends CommonActivity {
    ...
    public void initView() {
        binding.title.setOnBackIconClickListener(v -> onBackPressed())
                ...
                .setActionListener(v -> {
                    ...
                    List<QChatServerRoleInfo> list = rolesAdapter.getDataList().subList(firstPos, rolesAdapter.getItemCount());
                    //only one role will return directly
                    if (list.size() <= 1) {
                        finish();
                        return;
                    }
                    QChatRoleRepo.updateRolesPriorities(serverInfo.getServerId(), topPriority, list, new QChatCallback<Void>(getApplicationContext()) {
                        @Override
    ...
    private void initSort() {
        ItemTouchHelper.Callback touchCallback = new ItemTouchHelper.Callback() {
            ...
            public int getMovementFlags(@NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder) {
                QChatServerRoleViewHolder qChatServerRoleViewHolder = (QChatServerRoleViewHolder) viewHolder;
                final int dragFlags = qChatServerRoleViewHolder.isDisableSort() ? 0 : (ItemTouchHelper.UP | ItemTouchHelper.DOWN);
    ...
    private void getRolesList() {
        QChatRoleRepo.fetchServerRoles(serverInfo.getServerId(), 0, MAX_ROLE_PAGE_SIZE, new QChatCallback<ServerRoleResult>(getApplicationContext()) {
            @Override
            public void onSuccess(@Nullable ServerRoleResult param) {
                if (param != null && param.getRoleList() != null
                    ...
                    isServerOwner = TextUtils.equals(serverInfo.getOwner(), LoginService.INSTANCE.imAccId());
    ...
    private void showDeleteConfirmDialog(QChatServerRoleInfo roleInfo) {
        ...
        dialog.setNegativeStr(getResources().getString(R.string.qchat_cancel))
                .setTitleStr(getResources().getString(R.string.qchat_role_delete))
                .setContentStr(String.format(getResources().getString(R.string.qchat_role_delete_one_role), roleInfo.getName()))
    ...
    private void deleteRole(QChatServerRoleInfo roleInfo) {
        QChatRoleRepo.deleteServerRole(roleInfo.getServerId(), roleInfo.getRoleId(), new QChatCallback<Void>(getApplicationContext()) {
            @Override
            public void onSuccess(@Nullable Void param) {
