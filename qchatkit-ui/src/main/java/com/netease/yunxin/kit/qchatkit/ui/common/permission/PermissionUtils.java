...
public final class PermissionUtils {
    ...
    public static List<PermissionResult> shouldShowRequestPermissionRationale(Activity activity, String... permissions) {
        List<PermissionResult> results = new ArrayList<>();
        for (String permission : permissions) {
            results.add(new PermissionResult(permission, ActivityCompat.shouldShowRequestPermissionRationale(activity, permission)));
        }
        return results;
    }
    ...
    public static class PermissionRequest {
        ...
        public void request(PermissionCallback callback) {
            Objects.requireNonNull(context);
            TransHelper.launchTask(context, REQUEST_CODE_PERMISSION, (activity, integer) -> {
                ...
                callbackWrapper.onGranted(callback, intent.getStringArrayListExtra(KEY_PERMISSION_RESULT_GRANTED));

                List<String> deniedList = intent.getStringArrayListExtra(KEY_PERMISSION_RESULT_DENIED);
                List<String> deniedForeverList = intent.getStringArrayListExtra(KEY_PERMISSION_RESULT_DENIED_FOREVER);
                if ((deniedList != null && !deniedList.isEmpty()) || deniedForeverList != null && !deniedForeverList.isEmpty()) {
    ...
    private static class PermissionCallbackWrapper {
        ...
        public void onDenial(PermissionCallback callback, List<String> permissionsDenial, List<String> permissionDenialForever) {
            if (callback == null) {
                return;
            }
