...
/**
 * message view model
 * fetch and send message to channel
 */
public class MessageViewModel extends BaseViewModel {
    ...
    public void registerMessageObserver() {
        QChatServiceObserverRepo.observeReceiveMessage(mServerId, mChannelId, receiveMessageObserver, true);
    }
    ...
    private void queryMessage(long fromTime, long toTime, boolean reverse) {
        ...
        QChatMessageRepo.fetchMessageHistory(mServerId, mChannelId, fromTime, toTime, messagePageSize, reverse, new FetchCallback<List<QChatMessageInfo>>() {
            @Override
            public void onSuccess(@Nullable List<QChatMessageInfo> param) {
                ...
                if (fromTime == 0 && toTime == 0) {
                    ...
                    if (param != null && param.size() > 0 &&
                            TextUtils.equals(param.get(param.size() - 1).getUuid(), forwardMessage.getUuid())) {
            ...
            public void onException(@Nullable Throwable exception) {
                ...
                messageFetchResult.setError(QChatConstant.ERROR_CODE_MESSAGE_FETCH, R.string.qchat_channel_message_fetch_error);
                messageLiveData.postValue(messageFetchResult);
            }
        });
    }
    ...
    public QChatMessageInfo sendTextMessage(String content) {
        QChatSendMessageInfo sendMessageInfo = new QChatSendMessageInfo(mServerId, mChannelId, MsgTypeEnum.text, content);
        return sendMessage(sendMessageInfo);
    }

    public QChatMessageInfo sendImageMessage(ImageAttachment attachment) {
        QChatSendMessageInfo sendMessageInfo = new QChatSendMessageInfo(mServerId, mChannelId, MsgTypeEnum.image, null, null, attachment.toJson(false));
        return sendMessage(sendMessageInfo);

    }

    private QChatMessageInfo sendMessage(QChatSendMessageInfo sendMessageInfo) {
        return QChatMessageRepo.sendMessage(sendMessageInfo, new FetchCallback<QChatMessageInfo>() {
            @Override
            ...
            public void onException(@Nullable Throwable exception) {
                ...
                sendMessageFetchResult.setError(QChatConstant.ERROR_CODE_SEND_MESSAGE, R.string.qchat_channel_message_send_error);
                if (sendMessageInfo.getMessageInfo() != null){
                    sendMessageFetchResult.setData(sendMessageInfo.getMessageInfo());
                }
                sendMessageLiveData.postValue(sendMessageFetchResult);
    ...
    public void makeMessageRead(QChatMessageInfo messageInfo) {
        QChatMessageRepo.markMessageRead(messageInfo.getQChatServerId(), messageInfo.getQChatChannelId(), messageInfo.getTime(), new FetchCallback<Void>() {
            @Override
    ...
    private final EventObserver<List<QChatMessageInfo>> receiveMessageObserver = new EventObserver<List<QChatMessageInfo>>() {
        @Override
