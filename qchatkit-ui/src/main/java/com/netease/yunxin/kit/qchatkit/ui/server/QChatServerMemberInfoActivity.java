...
public class QChatServerMemberInfoActivity extends CommonActivity {
    ...
    public void initView() {
        ...
        member = (QChatServerMemberWithRoleInfo) getIntent().getSerializableExtra(QChatConstant.SERVER_MEMBER);
        ...
        binding.tvKick.setText(String.format(getResources().getString(R.string.qchat_kick_someone), nickname));
        binding.tvKick.setOnClickListener(v -> showKickDialog());
        binding.lyMore.setOnClickListener(v -> {
            if (rolesAdapter.getShowSub()) {
                binding.tvMore.setText(String.format(getResources().getString(R.string.qchat_pack_up_with_all), fullSize));
                binding.ivDown.setImageDrawable(ResourcesCompat.getDrawable(getResources(), R.drawable.ic_up, null));
                rolesAdapter.packUp();
            } else {
                binding.tvMore.setText(String.format(getResources().getString(R.string.qchat_more_with_all), fullSize));
                binding.ivDown.setImageDrawable(ResourcesCompat.getDrawable(getResources(), R.drawable.ic_down, null));
    ...
    private void updateMemberInfo() {
        ...
        if (TextUtils.equals(LoginService.INSTANCE.imAccId(), member.getAccId())) {
            ...
            QChatServerRepo.updateOtherMember(member.getServerId(), member.getAccId(), nick, new QChatCallback<Void>(getApplicationContext()) {
                @Override
                public void onSuccess(@Nullable Void param) {
    ...
    private void showKickDialog() {
        ...
        dialog.setTitleStr(String.format(getResources().getString(R.string.qchat_kick_someone), nickname))
                .setContentStr(String.format(getResources().getString(R.string.qchat_delete_some_member), nickname))
                ...
                .setConfirmListener(new ChoiceListener() {
                    ...
                    public void onPositive() {
                        QChatServerRepo.kickMember(member.getServerId(), member.getAccId(), new QChatCallback<Void>(getApplicationContext()) {
                            @Override
                            public void onSuccess(@Nullable Void param) {
    ...
    public void initData() {
        binding.avatar.setData(member.getAvatarUrl(), nickname, ColorUtils.avatarColor(member.getAccId()));
        binding.tvMember.setText(nickname);
        binding.edtMemberName.setText(member.getNick());
        initRolesList();
        getJoinedRoles();
        if (member.getType() == MEMBER_TYPE_OWNER
                || TextUtils.equals(LoginService.INSTANCE.imAccId(), member.getAccId())) {
    ...
    private void getJoinedRoles() {
        QChatRoleRepo.fetchMemberJoinedRoles(member.getServerId(), member.getAccId(), new QChatCallback<List<QChatServerRoleInfo>>(getApplicationContext()) {
            @Override
    ...
    private void getRoles() {
        QChatRoleRepo.fetchServerRoles(member.getServerId(), 0, MAX_ROLE_PAGE_SIZE, new QChatCallback<ServerRoleResult>(getApplicationContext()) {
            @Override
    ...
    private void initRolesList() {
        ...
        rolesAdapter = new CommonMoreAdapter<QChatServerRoleInfo, QChatRoleViewHolderBinding>() {

            @NonNull
            @Override
            public BaseMoreViewHolder<QChatServerRoleInfo, QChatRoleViewHolderBinding> getViewHolder(@NonNull ViewGroup parent, int viewType) {
                QChatRoleViewHolderBinding binding = QChatRoleViewHolderBinding.inflate(LayoutInflater.from(parent.getContext()), parent, false);
                return new BaseMoreViewHolder<QChatServerRoleInfo, QChatRoleViewHolderBinding>(binding) {
                    @Override
                    public void bind(QChatServerRoleInfo item) {
                        ...
                        if (!isEdit || item.getType() == QChatConstant.ROLE_EVERYONE_TYPE) {
                            ...
                            binding.rbCheck.setOnCheckedChangeListener((buttonView, isChecked) -> addOrRemoveMemberFromRoles(item, isChecked));
    ...
    private void addOrRemoveMemberFromRoles(QChatServerRoleInfo item, boolean add) {
        ...
        QChatCallback<List<String>> optionCallback = new QChatCallback<List<String>>(getApplicationContext()) {
            @Override
            ...
            public void onException(@Nullable Throwable exception) {
                Toast.makeText(getApplicationContext(), getString(R.string.qchat_server_request_fail) + exception, Toast.LENGTH_SHORT).show();
                rollbackRoles(item, add);
            }
        }.setProcess(binding.flyProcess);
        if (add) {
            joinedRoles.add(item);
            QChatRoleRepo.addServerRoleMember(member.getServerId(), item.getRoleId(), accIds, optionCallback);
        } else {
            joinedRoles.remove(item);
            QChatRoleRepo.removeServerRoleMember(member.getServerId(), item.getRoleId(), accIds, optionCallback);
        }
    }
