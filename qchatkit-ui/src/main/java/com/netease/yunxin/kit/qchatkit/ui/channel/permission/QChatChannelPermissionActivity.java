...
public class QChatChannelPermissionActivity extends CommonListActivity {
    ...
    public void initData() {
        ...
        viewModel.getRoleLiveData().observe(this, result -> {
        ...
        viewModel.getMemberLiveData().observe(this, result -> {
            if (result.getLoadStatus() == LoadStatus.Success) {
        ...
        viewModel.getRoleMoreLiveData().observe(this,result ->{
    ...
    private void checkAdapterData(){
        while (recyclerViewAdapter.getItemViewType(recyclerViewAdapter.getItemCount() - 1)  == QChatViewType.TITLE_VIEW_TYPE){
    ...
    public void initView() {
        ...
        viewBinding.commonActTitleView.setActionTextColor(getResources().getColor(R.color.color_337eff));
    }
    ...
    private void loadHeaderData() {
        ...
        String[] titleArray = new String[]{getResources().getString(R.string.qchat_channel_add_role_title), getResources().getString(R.string.qchat_channel_add_member_title)};
        String[] titleRouter = new String[]{QChatConstant.ROUTER_ADD_ROLE, QChatConstant.ROUTER_ADD_MEMBER};
    ...
    @Override
    public CommonViewHolder<QChatBaseBean> onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ALog.d(TAG,"onCreateViewHolder","viewType"+viewType);
        if (viewType == QChatViewType.TITLE_VIEW_TYPE) {
            QChatTitleViewholderLayoutBinding viewBinding = QChatTitleViewholderLayoutBinding.inflate(getLayoutInflater(), parent, false);
            return new TitleViewHolder(viewBinding);
        } else if (viewType == QChatViewType.ARROW_VIEW_TYPE) {
            QChatArrowViewHolderBinding viewBinding = QChatArrowViewHolderBinding.inflate(getLayoutInflater(), parent, false);
            ArrowViewHolder arrowViewHolder = new ArrowViewHolder(viewBinding);
            arrowViewHolder.setItemOnClickListener((data,position) -> {
            ...
            QChatMoreViewholderLayoutBinding viewBinding = QChatMoreViewholderLayoutBinding.inflate(getLayoutInflater(), parent, false);
            MoreViewHolder moreViewHolder = new MoreViewHolder(viewBinding);
            moreViewHolder.setItemOnClickListener((data,position) -> {
                viewModel.loadMoreRole();
            });
            return moreViewHolder;
        } else if (viewType == QChatViewType.CHANNEL_MEMBER_VIEW_TYPE) {
            QChatChannelMemberConerViewHolderBinding viewHolderBinding = QChatChannelMemberConerViewHolderBinding.inflate(getLayoutInflater(), parent, false);
            MemberViewHolder viewHolder = new MemberViewHolder(viewHolderBinding);
            viewHolder.setItemOnClickListener((data,position) -> { ;
                QChatMemberPermissionActivity.launch(this,((QChatChannelMemberBean) data).channelMember);
            });
            viewHolder.setOnDeleteClickListener(this::showDeleteDialog);
            return viewHolder;
        } else if (viewType == QChatViewType.CHANNEL_ROLE_VIEW_TYPE) {
            QChatChannelRoleViewHolderBinding viewBinding = QChatChannelRoleViewHolderBinding.inflate(getLayoutInflater(), parent, false);
            RoleViewHolder roleViewHolder = new RoleViewHolder(viewBinding);
            roleViewHolder.setItemOnClickListener((data,position) -> {
    ...
    private void showDeleteDialog(QChatBaseBean data,int position){
        ...
        String content = "";
        if (data instanceof QChatChannelRoleBean){
            content = String.format(getResources().getString(R.string.qchat_channel_role_delete_content),((QChatChannelRoleBean)data).channelRole.getName());
            title = getString(R.string.qchat_channel_role_delete_title);
        }else if (data instanceof QChatChannelMemberBean){
            content = String.format(getResources().getString(R.string.qchat_channel_member_delete_content),((QChatChannelMemberBean)data).channelMember.getNick());
