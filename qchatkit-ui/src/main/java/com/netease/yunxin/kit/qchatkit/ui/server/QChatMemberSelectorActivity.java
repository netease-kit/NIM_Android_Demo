...
public class QChatMemberSelectorActivity extends CommonActivity {
    ...
    protected void initView() {
        binding.title.setTitle(R.string.qchat_select)
        ...
        LinearLayoutManager selectorLayoutManager = new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false);
        binding.rvSelected.setLayoutManager(selectorLayoutManager);
        selectorAdapter = new CommonMoreAdapter<QChatServerRoleMemberInfo, QChatSelectorViewHolderBinding>() {
            @NonNull
            @Override
            public BaseMoreViewHolder<QChatServerRoleMemberInfo, QChatSelectorViewHolderBinding> getViewHolder(@NonNull ViewGroup parent, int viewType) {
                QChatSelectorViewHolderBinding binding = QChatSelectorViewHolderBinding.inflate(LayoutInflater.from(parent.getContext()), parent, false);
                return new BaseMoreViewHolder<QChatServerRoleMemberInfo, QChatSelectorViewHolderBinding>(binding) {
                    @Override
                    public void bind(QChatServerRoleMemberInfo item) {
                        ...
                        binding.avatar.setData(item.getAvatarUrl(), nick, ColorUtils.avatarColor(item.getAccId()));
        ...
        memberAdapter = new QChatServerMemberListAdapter((item, selected) -> {
            if (selected) {
                if (selectorAdapter.getItemCount() >= MAX_SELECTED_NUM) {
                    Toast.makeText(this, R.string.qchat_member_selector_max_count, Toast.LENGTH_LONG).show();
        ...
        decorator = new CommonMoreRecyclerViewDecorator<>(binding.rvMembers, memberLayoutManager, memberAdapter);
    ...
    private void getMembers(long timeTag) {
        ...
        QChatCallback<QChatPageResult<QChatServerRoleMemberInfo>> memberCallback = new QChatCallback<QChatPageResult<QChatServerRoleMemberInfo>>(this) {
            @Override
        ...
        switch (filterType) {
            case REQUEST_MEMBER_FILTER_ROLE:
                QChatServerRepo.fetchServerMembersWithRolesFilter(serverId, roleId, timeTag, PAGE_SIZE, memberCallback);
                break;
            case REQUEST_MEMBER_FILTER_CHANNEL:
                QChatServerRepo.fetchServerMembersWithWhiteBlackFilter(serverId,channelId,channelType,timeTag,PAGE_SIZE,memberCallback);
                break;
            default:
                //no filter
                QChatServerRepo.getServerMembersWithFilter(serverId, timeTag,
                        PAGE_SIZE, memberCallback);
                break;
        }
    }
