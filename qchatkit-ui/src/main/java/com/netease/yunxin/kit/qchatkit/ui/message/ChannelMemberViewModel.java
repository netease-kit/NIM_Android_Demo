...
import com.netease.yunxin.kit.qchatkit.repo.model.EventObserver;
...
public class ChannelMemberViewModel extends BaseViewModel {
    ...
    private final MutableLiveData<FetchResult<QChatChannelInfo>> channelInfoLiveData = new MutableLiveData<>();
    private final FetchResult<QChatChannelInfo> fetchChannelInfo = new FetchResult<>(LoadStatus.Finish);

    //member role info live data
    private final MutableLiveData<FetchResult<List<QChatServerRoleInfo>>> memberRoleLiveData = new MutableLiveData<>();
    private final FetchResult<List<QChatServerRoleInfo>> memberRoleResult = new FetchResult<>(LoadStatus.Finish);
    ...
    public void fetchMemberRoleList(long serverId, String accId) {
        ...
        QChatChannelRepo.fetchServerRolesByAccId(serverId, accId, 0, QChatConstant.MEMBER_PAGE_SIZE, new FetchCallback<List<QChatServerRoleInfo>>() {
            @Override
    ...
    public void fetchChannelInfo(long channelId) {
        QChatChannelRepo.fetchChannelInfo(channelId, new FetchCallback<List<QChatChannelInfo>>() {
            @Override
    ...
    /**
     * register channel change observer
     * include channel remove ,change ,member change and so on
     */
    public void registerDeleteChannelObserver(long serverId, long channelId) {
        ...
        QChatServiceObserverRepo.observerSystemNotificationWithType(notificationObserver, Arrays.asList(ChannelRemove.INSTANCE, ChannelUpdate.INSTANCE,
                ChannelUpdateWhiteBlackRoleMember.INSTANCE, ServerMemberInviteAccept.INSTANCE, ServerMemberApplyAccept.INSTANCE,
                ServerMemberKick.INSTANCE, ServerMemberLeave.INSTANCE, ServerMemberUpdate.INSTANCE));
    }
    ...
    private void fetchMemberData(long serverId, long channelId, long offset) {
        QChatChannelRepo.fetchChannelMembers(serverId, channelId, offset, QChatConstant.MEMBER_PAGE_SIZE, new FetchCallback<List<QChatServerMemberInfo>>() {
            @Override
            ...
            public void onException(@Nullable Throwable exception) {
                ...
                fetchResult.setError(QChatConstant.ERROR_CODE_CHANNEL_MEMBER_FETCH, R.string.qchat_channel_fetch_member_error);
                resultLiveData.postValue(fetchResult);
            }
        });
    }
    ...
    private final EventObserver<List<QChatSystemNotificationInfo>> notificationObserver = new EventObserver<List<QChatSystemNotificationInfo>>() {
        ...
        public void onEvent(@Nullable List<QChatSystemNotificationInfo> eventList) {
            ...
            for (QChatSystemNotificationInfo item : eventList) {
                ...
                ALog.d(TAG, "notificationObserver", "info:" + serverId + "," + channelId + "," + type.toString());
                ...
                } else if (serverId == observerServerId && channelId == observerChannelId &&
                        type == ChannelUpdateWhiteBlackRoleMember.INSTANCE || type == ServerMemberInviteAccept.INSTANCE || type == ServerMemberApplyAccept.INSTANCE ||
                        type == ServerMemberKick.INSTANCE || type == ServerMemberLeave.INSTANCE || type == ServerMemberUpdate.INSTANCE) {
