...
public class AddMemberViewModel extends BaseViewModel {
    ...
    private void fetchMemberData(long serverId, long channelId, long offset) {
        QChatServerRepo.fetchServerMemberWithoutChannel(serverId, channelId, offset, QChatConstant.MEMBER_PAGE_SIZE, new FetchCallback<List<QChatServerMemberInfo>>() {
            @Override
    ...
    public void addMemberToChannel(long serverId, long channelId, String accId) {
        QChatRoleRepo.addChannelMemberRole(serverId, channelId, accId, new FetchCallback<QChatChannelMember>() {
            @Override
            ...
            public void onFailed(int code) {
                ...
                addResult.setError(code, ErrorUtils.getErrorText(code,R.string.qchat_channel_add_member_error));
                addLiveData.postValue(addResult);
            }
            ...
            public void onException(@Nullable Throwable exception) {
                ...
                addResult.setError(QChatConstant.ERROR_CODE_CHANNEL_MEMBER_ADD, R.string.qchat_channel_add_member_error);
                addLiveData.postValue(addResult);
            }
        });
    }
    ...
    public boolean memberFilter(String accId) {
        return !TextUtils.equals(accId, LoginService.INSTANCE.imAccId());
    }
