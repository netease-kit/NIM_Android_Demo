...
public class PullToRefreshListView extends PullToRefreshAdapterViewBase<ListView> {
	...
	protected ListView createRefreshableView(Context context, AttributeSet attrs) {
		...
		// 避免app切换到后台（可能被destroy）导致数据列表被清空而updateView中crash。
		// 使用android.R.id.list也容易冲突。by mamingli
		lv.setId(R.id.pullToRefresh_internalViewId);
		return lv;
	}
	...
	protected boolean isLoadingViewEnabled(boolean header) {
		return true;
	}
	...
	protected class InternalListView extends ListView implements EmptyViewMethodAccessor {
		...
		public void setAdapter(ListAdapter adapter) {
			...
			if (null != mLvFooterLoadingFrame && !mAddedLvFooter) {
				// enable footer loading view
				if (isLoadingViewEnabled(false)) {
