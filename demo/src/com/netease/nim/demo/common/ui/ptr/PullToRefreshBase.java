 ...
 *******************************************************************************/
package com.netease.nim.demo.common.ui.ptr;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public OnHeaderScrollListener headerScrollListener;
	...
	public final boolean demo() {
		if (mMode.showHeaderLoadingLayout() && isReadyForPullStart()) {
			smoothScrollToAndBack(-getHeaderSize(false) * 2);
			return true;
		} else if (mMode.showFooterLoadingLayout() && isReadyForPullEnd()) {
			smoothScrollToAndBack(getFooterSize(false) * 2);
			return true;
		}
	...
	final void setState(State state, final Object... params) {
		...
		switch (mState) {
			...
			case PULL_TO_REFRESH:
				onPullToRefresh((Integer) params[0], (Integer) params[1]);
				break;
	...
	protected int getFooterSize(boolean pull) {
	...
	protected int getHeaderSize(boolean pull) {
	 ...
	 */
	protected void onPullToRefresh(int scrollValue, int pullThreshold) {
		switch (mCurrentMode) {
			case PULL_FROM_END:
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				...
				switch (mCurrentMode) {
					...
					case PULL_FROM_END:
						smoothScrollTo(getFooterSize(false), listener);
						break;
					default:
					case PULL_FROM_START:
						smoothScrollTo(-getHeaderSize(false), listener);
						break;
				}
			} else {
				smoothScrollTo(0);
...
//		}
		...
		onHeaderScrol(isReadyForPullStart(),value);
	}
	...
	private void pullEvent() {
		...
		switch (mCurrentMode) {
			case PULL_FROM_END:
				...
				itemDimension = getFooterSize(true);
				break;
			case PULL_FROM_START:
			default:
				newScrollValue = Math.round(Math.min(initialMotionValue - lastMotionValue, 0) / FRICTION);
				itemDimension = getHeaderSize(true);
				break;
		}
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			if (itemDimension >= Math.abs(newScrollValue)) {
				setState(State.PULL_TO_REFRESH, Math.abs(newScrollValue), itemDimension);
	...
	public interface OnHeaderScrollListener {
		...
		public void onHeaderScroll(boolean isRefreahing,boolean isTop, int value);
	}
