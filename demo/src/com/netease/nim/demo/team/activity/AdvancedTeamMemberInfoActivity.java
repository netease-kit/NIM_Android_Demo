...
import com.netease.nim.demo.common.ui.dialog.MenuDialog;
...
public class AdvancedTeamMemberInfoActivity extends TActionBarActivity implements View.OnClickListener {
    ...
    private void parseIntentData() {
        uid = getIntent().getStringExtra(EXTRA_ID);
    ...
    private void loadMemberInfo() {
        viewMember = TeamDataCache.getInstance().getTeamMemberByAccount(teamId, uid);
        if (viewMember != null) {
            updateMemberInfo();
        } else {
            requestMemberInfo();
    ...
    private void requestMemberInfo() {
        NIMClient.getService(TeamService.class).queryTeamMember(teamId, uid).setCallback(new RequestCallbackWrapper<TeamMember>() {
            @Override
    ...
    private void initMemberInfo() {
        memberName.setText(ContactDataCache.getInstance().getUserDisplayName(uid));
        headImageView.loadBuddyAvatar(uid);
    }
    ...
    private void updateSelfIndentity() {
        TeamMember selfTeamMember = TeamDataCache.getInstance().getTeamMemberByAccount(teamId, DemoCache.getAccount());
    ...
    private void updateRemoveBtn() {
        if (viewMember.getAccount().equals(DemoCache.getAccount())) {
    ...
    private void setNickname(final String name) {
        ...
        NIMClient.getService(TeamService.class).updateMemberNick(teamId, uid, name).setCallback(new RequestCallback<Void>() {
            @Override
    ...
    private void removeMember() {
        DialogMaker.showProgressDialog(this, getString(R.string.empty));
        NIMClient.getService(TeamService.class).removeMember(teamId, uid).setCallback(new RequestCallback<Void>() {
            @Override
            public void onSuccess(Void param) {
                DialogMaker.dismissProgressDialog();
                makeIntent(uid, isSetAdmin, true);
                finish();
                Toast.makeText(AdvancedTeamMemberInfoActivity.this, R.string.update_success, Toast.LENGTH_LONG).show();
            }
    ...
    public void onBackPressed() {
        makeIntent(uid, isSetAdmin, false);
        super.onBackPressed();
    }
    ...
    private boolean isSelf(String uid) {
        return DemoCache.getAccount().equals(uid);
    }
}
