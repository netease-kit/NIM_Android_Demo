...
import com.netease.nim.demo.team.viewholder.TeamAnnounceHolder;
...
public class AdvancedTeamAnnounceActivity extends TActionBarActivity implements TAdapterDelegate {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.advanced_team_announce);
    ...
    private void requestMemberData() {
        TeamMember teamMember = TeamDataCache.getInstance().getTeamMemberByAccount(teamId, DemoCache.getAccount());
        if (teamMember != null) {
            updateTeamMember(teamMember);
        } else {
            // 请求群成员
            NIMClient.getService(TeamService.class).queryTeamMember(teamId, DemoCache.getAccount()).setCallback(new RequestCallbackWrapper<TeamMember>() {
                @Override
