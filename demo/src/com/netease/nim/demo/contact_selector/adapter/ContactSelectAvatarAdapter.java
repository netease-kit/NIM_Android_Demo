...
public class ContactSelectAvatarAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(context).inflate(R.layout.contact_select_area_item, null);
		...
		try {
			...
			if (item == null) {
				imageView.setBackgroundResource(R.drawable.dot_avatar);
	...
	public void removeContact(IContact contact) {
		...
		for (Iterator<IContact> iterator = selectedContactItems.iterator(); iterator.hasNext();) {
			IContact iContact = (IContact) iterator.next();
			if (iContact == null) {
				continue;
			}
